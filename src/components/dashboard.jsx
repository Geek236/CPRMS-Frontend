...
const Dashboard = () => {
  ...
  const fetchUserCars = async (username) => {
    try {
      const res = await axios.post(`${API_URL}/Cars/SpecificCar`, {
        FullName: username,
      });
  ...
  const fetchServiceRecords = async (username) => {
    try {
      const res = await axios.post(
        `${API_URL}/ServiceRecords/getRecordsByUser`,
        { FullName: username },
      );
  ...
  const handleRequestService = async () => {
    ...
    try {
      const res = await axios.post(
        `${API_URL}/ServiceRecords/createRecord`,
        {
          PlateNumber: plateNumber,
          FullName: fullName,
  ...
  const handleDeleteRecord = async (record) => {
    try {
      if (!record.StatusOfService) {
        await axios.delete(
          `${API_URL}/ServiceRecords/deleteRecord`,
          {
